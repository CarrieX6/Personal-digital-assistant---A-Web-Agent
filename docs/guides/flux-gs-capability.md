...
- 单一飞书功能菜单中的“3D 场景建模”入口，以及点击后的普通文本状态说明；
...
### 3.1 与产品“单张 2D 图片转 3D”的边界

当前上游仓库没有实现单图直接建模。真正的产品流水线应当是：

```text
飞书上传单张图片
  → 图片格式、安全与主体完整性校验
  → 单图生成一致多视角（待选型、待接入）
  → 相机位姿估计与 COLMAP 数据构建（待实现）
  → Flux-GS 逐场景训练与压缩
  → 发布 WebGL Viewer
  → 飞书返回预览链接或降级视频
```

因此 Flux-GS 是这条流水线的后半段，而不是完整的单图 2D→3D 模型。后续应新增面向
用户的 `create_image_to_3d` 编排工具，由它消费飞书图片资产并串联各阶段；现有
`create_flux_gs_demo` 保留为内部多视角训练工具。在前置模型通过质量、延迟、显存和
许可证评测前，不能把统一菜单入口描述为已经支持单图建模。
...
1. 向机器人发送“菜单”，机器人只返回一张统一功能菜单；
2. 点击“3D 场景建模”；
3. 机器人通过普通文本展示 Provider 是否就绪、COLMAP 输入要求和调用格式，不再发送
   第二张功能卡；
4. 发送 `生成 Flux-GS，dataset_id=scene-a`；
5. 在审批入口确认 GPU 训练；
6. 任务完成后取得 `demo_url`，在手机浏览器打开 WebGL Viewer。

飞书菜单不是 3D 渲染容器，只负责入口。状态和错误使用普通消息，最终结果使用链接或
结果消息。`demo_url` 必须是手机可访问的 HTTPS 地址；`127.0.0.1`、Mac 本地路径和仅
GPU 内网可见的 URL 都不能回传给手机。
