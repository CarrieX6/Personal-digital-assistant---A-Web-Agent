...
- “图片风格化”：先发送 1 张内容图，再发送 1–3 张风格参考图，点击“开始生成”；完成
  后机器人直接返回结果图片和模型参数卡片。
- “Flux-GS 3D”：显示独立 GPU Provider 状态、COLMAP 数据要求和调用格式。管理员预先
  放置数据集后发送 `生成 Flux-GS，dataset_id=scene-a`，训练创建必须人工审批；完成
  后通过手机可访问的 HTTPS `demo_url` 打开 WebGL 预览。
...
Flux-GS 是多视角 3D 重建能力，不接收单张图片。飞书卡片只展示入口、服务状态和最终
链接；训练在独立 Linux/NVIDIA GPU 服务执行。数据集准备、受控目录映射、安全边界和
许可证门禁见 [Flux-GS Capability 接入与部署](flux-gs-capability.md)。当前尚未支持在
飞书中直接上传大型 COLMAP ZIP，不能把手机文件路径作为 `dataset_id`。
