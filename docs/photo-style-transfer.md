...
2. 选择自动、中国水墨、赛博朋克、经典油画、后印象派、水彩、动漫或电影预设；
3. 选择“保留布局”或“重新构图”、质量档位及三项强度；
4. `POST /api/photo-style-transfers` 返回资产和异步任务；
5. 前端轮询 `/api/jobs/{job_id}`；
6. 完成后并排显示原图与风格化结果，并提供本地下载；
7. 结果进入与空间照片共用的 SQLite/文件资产库。

参考图文件名包含“水墨 / 赛博 / 梵高 / 水彩 / 油画 / 动漫 / 电影”等关键词时，网页会
自动选择对应预设，仍可手动修改。预设不是简单地附加一个风格词：它会把媒介描述放在
SDXL 文本窗口前部，保留内容图主体与几何的契约，限制参考图主体泄漏，并按风格分别映射
IP-Adapter、去噪、引导和调色强度。用户填写补充描述时不会再覆盖上述基础契约。
...
| `style_preset` | `auto` / `ink_wash` / `cyberpunk` / `oil_painting` / `post_impressionist` / `watercolor` / `anime` / `cinematic` | 典型媒介与风格的专属提示词、参数映射和调色策略 |
...
本机 Provider 的标准档由 16 个有效步提升为 20 个；高质量档使用 28 个有效步，并把最长边
提高到 896。尺寸现在按 8 像素对齐并保持原始宽高比，不再按 64 像素粗粒度取整造成主体
横向或纵向拉伸。水墨预设额外执行克制的宣纸色调、墨色分层和轮廓融合；赛博等彩色预设
根据参考图进行有限的色板协调，但不改变内容图几何。
