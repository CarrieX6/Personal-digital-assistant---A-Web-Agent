...
| `PREVIEW-001` | P1 | 手机如何安全预览空间照片、GLB、PLY 和 3DGS | `🟠 初步结论` | `🟡 开发中` | Zhuofan Xie | 已实现独立 `8765` 只读 Viewer、HMAC 短时链接、文件白名单、触控/陀螺仪视差与安全响应头；待 iOS/Android 真机、防火墙、弱网和链接撤销测试，GLB/3DGS 未接入 |
| `REMOTE-001` | P1 | 局域网、隧道、中继和远程唤醒如何选择 | `🟠 初步结论` | `🟡 开发中` | Zhuofan Xie | 局域网仅按需暴露签名 Viewer，不暴露 Root API；公网阶段需域名、HTTPS、认证、撤销、限流、审计与中继，尚未实施 |
...
| `STYLE-001` | P2 | 图片个性化如何在 CPU 预览、SDXL 本机与远端 Provider 间切换 | `🟠 初步结论` | `🟡 开发中` | Ma Xianggang / Zhuofan Xie | 已将旧 `feature/pic-style` 的 Service、Provider、模型门禁、测试和工作台迁入新版工具库与 Agent；默认 CPU 仅用于预览，真实 SDXL 仍需目标 Windows GPU 复验和合并评审 |
