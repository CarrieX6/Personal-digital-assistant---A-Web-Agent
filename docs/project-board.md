...
| ID | 优先级 | 调研问题 | 调研状态 | 实现状态 | 负责人 | 交付与验收 |
| --- | --- | --- | --- | --- | --- | --- |
| `CHANNEL-001` | P0 | 飞书长连接、权限、白名单、幂等和文本回复 | `🟠 初步结论` | `🟡 开发中` | Zhuofan Xie | 已实现配置 UI、安全凭证、长连接、SQLite 去重、Open ID 白名单和文本回复；新增 [飞书配置与使用指南](guides/feishu-setup.md)及 Web 可打开入口，明确事件/回调、可用范围、群聊和图片排障；待断线重连验收与代码合并 |
| `CHANNEL-002` | P1 | 图片、文件、视频、卡片、进度和失败如何回传 | `🟡 调研中` | `🟡 开发中` | Zhuofan Xie | 已实现单图下载、空间任务、封面回传、Markdown 富文本回复、JSON 2.0 功能卡片及按钮回调；待真实卡片权限实测、多图、文件、视频与交互 Viewer |
...
| `CAP-001` | P0 | Capability Manifest、权限和调用接口如何定义 | `🟠 初步结论` | `🟡 开发中` | Zhuofan Xie | 2026-07-29 完成工具库 UI 状态模型（已安装/未安装/待上线），空间照片移入工具库；待 Manifest v1、真实安装器、Schema 校验、版本/依赖/许可与卸载流程 |
...
| `OPS-001` | P1 | 本地 Agent 如何开机启动、守护、升级和告警 | `⬜ 未开始` | `⬜ 未开始` | 待领取 | launchd/systemd、健康检查、日志轮转、磁盘告警和升级回滚 |
