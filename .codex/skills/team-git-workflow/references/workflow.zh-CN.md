...
将 `docs/project-board.md` 作为项目调研与实现状态的统一入口。文件存在时：

1. 从用户请求、分支、Issue、PR 或调研文档中识别任务 ID。
2. 开始任务时读取对应条目，确认优先级、负责人、依赖、调研状态和实现状态。
3. 项目级调研或实现没有对应条目时，在范围明确后分配唯一 ID 并登记；格式调整、
   纯分支清理等维护操作不强制创建看板任务。
4. 在同一任务分支中同步已经发生的事实：
   - 领取任务：填写负责人，调研进入 `🟡 调研中` 或实现进入 `🟡 开发中`；
   - 新增调研、实验或 ADR：更新状态和文档链接；
   - 创建 PR：实现状态改为 `👀 评审中` 并记录真实 PR 链接；
   - 确认合并且验收通过：实现状态才改为 `✅ 已完成`。
5. PR 链接只有创建后才能获得。看板需要该链接时，在同一分支追加一个小型文档
   提交并再次推送。
6. 提交前比较看板和实际状态；只更新当前任务，不猜测、不批量改写其他任务。
7. 不因创建了提交或 PR 就标记完成，不得写入尚未发生的合并、测试或评审结果。
8. 看板不在当前分支时报告依赖关系，不创建失效链接，也不复制一份临时看板。
...
只有 GitHub 已确认 PR 合并，并且目标分支准确时，才按以下顺序执行：

1. 合并 PR；
2. 删除对应远程功能分支；
3. 切换到本地 `main`，运行 `git pull --ff-only origin main`；
4. 使用 `git branch -d <branch>` 删除对应本地功能分支；
5. 运行 `git fetch origin --prune`，确认远程、本地分支和工作区状态。

不得删除未合并、仍被其他成员使用或包含未发布提交的分支。合并后的看板状态尚未
更新时，使用独立文档分支补充 `✅ 已完成` 和合并链接；未经授权不直接提交到
`main`。
